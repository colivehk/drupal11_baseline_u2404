...
SCRIPT_VERSION="${SCRIPT_VERSION:-V0.1.2.8}"
...
as_app_user_www() {
  # Hard mode: everything runs from ${DRUPAL_DIR}
  as_app_user_drupal "$@"
}
...
if COMPOSER_DIAG="$(
  as_app_user_drupal "env -i HOME=\"/var/www\" PATH=\"${SAFE_PATH}\" COMPOSER_HOME=\"${COMPOSER_HOME_DIR}\" COMPOSER_CACHE_DIR=\"/var/www/.cache/composer\" ${COMPOSER_BIN} --working-dir="${DRUPAL_DIR}" diagnose --no-ansi"
)"; then
  DIAG_EC=0
else
  DIAG_EC=$?

...
SCRIPT_VERSION="${SCRIPT_VERSION:-V7.5.10.7}"
...
INSTALL_PROFILE_DEFAULT="${INSTALL_PROFILE_DEFAULT:-minimal}"  # fallback when non-interactive
...
print_help(){
  cat <<'EOF'
Usage:
  sudo -E bash drupal_installV7.5.10.7.sh [--profile <name>] [--list-profiles] [--fix-perms] [--no-acl]

Options:
  --profile, -p <name>  Drupal install profile (e.g. standard|minimal|demo_umami|...). Any profile present under web/*/profiles.
  --list-profiles       Print detected install profiles (after codebase is present) and exit
  --fix-perms           Only normalize ownership/permissions on existing paths (no apt, no install, no mkdir)
  --acl                 Apply ACL on sites/default/files for OPS_USER (default: enabled)
  --no-acl              Disable ACL application
  -h, --help            Show this help

Notes:
  - Root is required.
  - If no profile is provided and you run in an interactive terminal, the script will prompt you to choose (default prefers 'standard' if available).
  - Non-interactive default fallback: INSTALL_PROFILE_DEFAULT (default: minimal).
  - Default model: CODE_OWNER=ubuntu, OPS_USER=ubuntu, WEB_USER=www-data (APP_USER works), COLLAB_GROUP=drupal
  - Full install mode installs 'acl' package so setfacl/getfacl are available (ACL is applied by default unless you pass --no-acl)
...
########################################
# Install profile helpers
########################################
# Profiles live under:
#   - ${WEB_ROOT}/core/profiles (core: standard/minimal/demo_umami, etc.)
#   - ${WEB_ROOT}/profiles (custom/distribution)
#   - ${WEB_ROOT}/profiles/contrib, ${WEB_ROOT}/profiles/custom
list_install_profiles() {
  local -a found=()
  local dir
  for dir in "${WEB_ROOT}/core/profiles" "${WEB_ROOT}/profiles" "${WEB_ROOT}/profiles/contrib" "${WEB_ROOT}/profiles/custom"; do
    [[ -d "${dir}" ]] || continue
...
profile_exists() {
  local p="$1"
  [[ -d "${WEB_ROOT}/core/profiles/${p}" \
  || -d "${WEB_ROOT}/profiles/${p}" \
  || -d "${WEB_ROOT}/profiles/contrib/${p}" \
  || -d "${WEB_ROOT}/profiles/custom/${p}" ]]
}

resolve_install_profile() {
  local default_profile="${INSTALL_PROFILE_DEFAULT:-minimal}"
  local chosen="${INSTALL_PROFILE:-}"

  local -a profs=()
  mapfile -t profs < <(list_install_profiles || true)
...
' "${profs[@]}" | sed 's/^/ - /'
  ...
  # If user provided a profile but it's invalid, show choices and (if interactive) prompt again.
  if [[ -n "${chosen}" ]] && ! profile_exists "${chosen}"; then
    warn "Requested install profile '${chosen}' not found under ${WEB_ROOT}."
...
' "${profs[@]}" | sed 's/^/ - /'
    if [[ -t 0 ]]; then
      ...
      die "Invalid INSTALL_PROFILE='${chosen}'. Set a valid profile name or use --list-profiles."
    fi
  fi

  if [[ -z "${chosen}" ]]; then
    if [[ -t 0 ]]; then
      # Prefer 'standard' as the interactive default if present (better UX for building a site).
      local interactive_default="${default_profile}"
      if printf '%s
' "${profs[@]}" | grep -Fxq "standard"; then
        interactive_default="standard"
      fi

      warn "No install profile specified. Minimal is lean but requires manual enabling of modules; Standard is usually faster for building a site (Threads-like)."
      echo
      echo "Select Drupal install profile:"
      local i=1
      for p in "${profs[@]}"; do
        printf "  [%d] %s
" "$i" "$p"
      ...
      local ans=""
      read -r -p "Enter number or name [default: ${interactive_default}]: " ans || true
      ans="${ans:-${interactive_default}}"

      if [[ "${ans}" =~ ^[0-9]+$ ]]; then
        local n="${ans}"
        (( n >= 1 && n <= ${#profs[@]} )) || die "Invalid selection: ${ans}"
        chosen="${profs[n-1]}"
      else
        chosen="${ans}"
      fi
  ...
  if ! profile_exists "${chosen}"; then
    warn "Chosen profile '${chosen}' not found; falling back to '${default_profile}'."
    chosen="${default_profile}"
  fi
  profile_exists "${chosen}" || die "Could not resolve a valid install profile. Use --list-profiles to see available choices."
  INSTALL_PROFILE="${chosen}"
...
ensure_collab_group() {
  # Track whether WEB_USER was already in the collaboration group BEFORE we touch anything.
  # If we add WEB_USER to the group after php-fpm is already running, php-fpm workers
  # won't pick up the new supplementary group until php-fpm is restarted.
  local web_before=0 web_after=0

  if id -u "${WEB_USER}" >/dev/null 2>&1; then
    id -nG "${WEB_USER}" | tr ' ' '
' | grep -Fxq "${COLLAB_GROUP}" && web_before=1 || web_before=0
  fi
  ...
  for u in "${CODE_OWNER}" "${OPS_USER}" "${WEB_USER}"; do
    if id -u "${u}" >/dev/null 2>&1; then
      # Only add if not already a member (keeps logs cleaner + avoids needless restarts)
      if ! id -nG "${u}" | tr ' ' '
' | grep -Fxq "${COLLAB_GROUP}"; then
        usermod -aG "${COLLAB_GROUP}" "${u}" || true
        added_users+=("${u}")
        [[ "${u}" == "${WEB_USER}" ]] && web_added=1
      fi
    else
  ...
  if (( ${#added_users[@]} > 0 )); then
    warn "Added users to group '${COLLAB_GROUP}': ${added_users[*]}. Existing SSH sessions won't see it until you re-login (or run: newgrp ${COLLAB_GROUP}). If you hit drush 'Permission denied' on files/, re-run with --acl."
  fi

  if id -u "${WEB_USER}" >/dev/null 2>&1; then
    id -nG "${WEB_USER}" | tr ' ' '
' | grep -Fxq "${COLLAB_GROUP}" && web_after=1 || web_after=0
  fi

  # Full install mode only: if WEB_USER just gained the group and php-fpm is already active,
  # restart php-fpm so workers inherit the new supplementary group immediately.
  if [[ "${FIX_PERMS_ONLY}" != "1" && "${web_before}" == "0" && "${web_after}" == "1" ]]; then
    if systemctl is-active --quiet php8.3-fpm 2>/dev/null; then
      step "BASELINE: php-fpm group refresh (restart php8.3-fpm for ${WEB_USER})"
      systemctl restart php8.3-fpm
      ok "php8.3-fpm restarted to pick up group '${COLLAB_GROUP}' for ${WEB_USER}"
    fi
  fi
}
...
harden_sites_default() {
  ...
  [[ -d "${sd}" ]] || return 0
  step "HARDEN: protect sites/default and settings files"

  chown "${OPS_USER}:${COLLAB_GROUP}" "${sd}" 2>/dev/null || true
  ...
  for f in settings.php settings.statekeeper.php settings.redis.php; do
    if [[ -f "${sd}/${f}" ]]; then
      chown "${OPS_USER}:${COLLAB_GROUP}" "${sd}/${f}" 2>/dev/null || true
...
atomic_set_managed_block(){
  local _file="$1"
  local _label="$2"
  [[ -n "$_file" && -n "$_label" ]] || return 0
  local _tmp
  _tmp="$(mktemp /tmp/managed-block.XXXXXX)"
  cat >"$_tmp"
  python3 - "$_file" "$_label" "$_tmp" <<'PY'
import os, re, sys, tempfile
path, label, content_path = sys.argv[1], sys.argv[2], sys.argv[3]
...
with open(content_path, "r", encoding="utf-8", errors="surrogateescape") as f:
    content = f.read().splitlines()

begin_line = f"# BEGIN {label}\n"
end_line   = f"# END {label}\n"

# Remove any existing blocks (all occurrences)
out = []
i = 0
n = len(lines)
while i < n:
    line = lines[i]
    if line.strip() == begin_line.strip():
        # skip until matching end
        j = i + 1
        while j < n and lines[j].strip() != end_line.strip():
            j += 1
        if j < n:
            j += 1
            i = j
...
# Remove duplicate PHP open tags (keep first occurrence)
php_re = re.compile(r'^\s*<\?php\b')
seen_php = False
out2 = []
for line in out:
    if php_re.match(line):
        if seen_php:
            continue
        seen_php = True
    out2.append(line)
...
if out2 and not out2[-1].endswith("\n"):
    out2[-1] += "\n"

# Append block (ensure one blank line separation)
if out2 and out2[-1].strip() != "":
    out2.append("\n")
out2.append(begin_line)
for l in content:
    out2.append(l + "\n")
out2.append(end_line)

# Atomic write with perms preserved if possible
d = os.path.dirname(path) or "."
fd, tmp = tempfile.mkstemp(prefix=".tmp_block_", dir=d)
os.close(fd)
...
# NOTE: php8.3-fpm start is intentionally delayed until after collaboration group baseline
...
service_active php8.3-fpm && systemctl reload php8.3-fpm || true
...
########################################
# 6) Start php-fpm (delayed until after group baseline)
########################################
step "BASELINE: starting php8.3-fpm (delayed until after group baseline)"
...
########################################
# 12) Site install (selected profile) if needed
########################################
# Ensure sites/default/files exists and follows the V7.5.2 ownership model
install -d -m 0755 -o "${CODE_OWNER}" -g "${COLLAB_GROUP}" "${WEB_ROOT}/sites/default"
install -d -m 02775 -o "${WEB_USER}" -g "${COLLAB_GROUP}" "${WEB_ROOT}/sites/default/files"
normalize_permissions
...
if [[ "$INSTALLED" == "1" ]]; then
  ...
  resolve_install_profile
  step "INSTALL: running drush site:install (${INSTALL_PROFILE}) as ${OPS_USER}"
  as_app_user_drupal "./vendor/bin/drush -y site:install ${INSTALL_PROFILE} \
...
# Ensure settings.php has safe file_chmod_* BEFORE first request
if [[ -f "${WEB_ROOT}/sites/default/settings.php" ]]; then
  step "Writing file_chmod_* safety settings into settings.php"
  atomic_set_managed_block "${WEB_ROOT}/sites/default/settings.php" "DRUPAL_TOOLS_FILE_CHMOD" <<'PHP'
$settings['file_chmod_directory'] = 02775;
$settings['file_chmod_file'] = 0664;
PHP
  dedupe_managed_blocks_file "${WEB_ROOT}/sites/default/settings.php" || true
fi

...
VERSION="drupal_statekeeperV0.1.4.1"
...
Usage:
  Snapshot:
    sudo -E bash drupal_statekeeperV0.1.4.1.sh snapshot [--incremental] [--force] [--with-files] [--with-db] [--acl]
      [--uri=http://IP] [--drupal-dir=/var/www/drupal]

  Apply:
    sudo -E bash drupal_statekeeperV0.1.4.1.sh apply --from=/path/to/drupal_snapshot_YYYYmmdd_HHMMSS.tar.gz
      [--restore-files] [--restore-db] [--acl] [--uri=http://IP] [--drupal-dir=/var/www/drupal]

Notes:
  - Default model: OPS_USER=ubuntu runs composer/drush; WEB_USER=www-data writes sites/default/files.
  - Optional ACL mode (--acl): grant OPS_USER rwX on sites/default/files even if group membership isn't refreshed.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --acl) ACL_MODE=1; shift ;;
    --no-acl) ACL_MODE=0; shift ;;
    --acl-clear) ACL_CLEAR=1; shift ;;
...
files_dir() {
  echo "$DRUPAL_DIR/web/sites/default/files"
}

apply_acl_files() {
  [[ "$ACL_MODE" -eq 1 ]] || return 0
  local d; d="$(files_dir)"
  if [[ ! -d "$d" ]]; then
    warn "ACL requested but files dir not found ($d); skipping."
    return 0
  fi
  if ! acl_tools_present; then
    warn "ACL requested but 'setfacl/getfacl' not found. Install package: sudo apt-get install -y acl"
    return 0
  fi
  step "ACL: granting $OPS_USER rwX on $d (and default ACL for new files)"
  setfacl -R -m "u:${OPS_USER}:rwX" "$d" || true
  setfacl -R -d -m "u:${OPS_USER}:rwX" "$d" || true
  ok "ACL applied on files dir."
}

clear_acl_files() {
  [[ "$ACL_CLEAR" -eq 1 ]] || return 0
  local d; d="$(files_dir)"
  if [[ ! -d "$d" ]]; then
    warn "ACL clear requested but files dir not found ($d); skipping."
    return 0
  fi
  if ! acl_tools_present; then
    warn "ACL clear requested but 'setfacl/getfacl' not found. Install package: sudo apt-get install -y acl"
    return 0
  fi
  step "ACL: clearing ACL recursively on $d"
  setfacl -Rb "$d" || true
  ok "ACL cleared on files dir."
}
...
PHP
  ...
  # Ensure operator can edit this file (avoid root-owned pollution).
  chown "$OPS_USER:$COLLAB_GROUP" "$DRUPAL_DIR/web/sites/default/settings.statekeeper.php" 2>/dev/null || true
  chmod 0664 "$DRUPAL_DIR/web/sites/default/settings.statekeeper.php" 2>/dev/null || true
}

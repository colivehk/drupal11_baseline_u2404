...
SCRIPT_VERSION="${SCRIPT_VERSION:-V0.1.2.14}"
SCRIPT_FLAVOR="${SCRIPT_FLAVOR:-hard_single_cwd}"
...
ensure_drupal_root() {
  ...
  # Composer runtime dirs for ${APP_USER}
  install -d -m 02775 -o "${APP_USER}" -g "${APP_USER}" "${COMPOSER_HOME_DIR}"
  install -d -m 02775 -o "${APP_USER}" -g "${APP_USER}" "/var/www/.cache"
  install -d -m 02775 -o "${APP_USER}" -g "${APP_USER}" "/var/www/.cache/composer"
}

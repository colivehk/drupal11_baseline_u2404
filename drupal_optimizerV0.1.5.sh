...
optimize_nginx(){
  ...
  step "3) Nginx global tuning (safe include, no gzip on)"
  ...
  # Build conf with only directives that are NOT already present (avoid 'duplicate directive' errors)
  {
    ...
    # gzip: auto enables only if no gzip directive exists already
    if [[ "${ENABLE_NGINX_GZIP}" == "1" ]]; then
      if ! nginx_has "gzip"; then echo "gzip on;"; fi
      if ! nginx_has "gzip_comp_level"; then echo "gzip_comp_level 5;"; fi
      if ! nginx_has "gzip_min_length"; then echo "gzip_min_length 1024;"; fi
      if ! nginx_has "gzip_types"; then
        cat <<'EOF_GZIP'
gzip_types
  text/plain
  text/css
...
EOF_GZIP
    ...
    elif [[ "${ENABLE_NGINX_GZIP}" == "auto" ]]; then
      if ! nginx_has "gzip"; then
        echo "gzip on;"
        if ! nginx_has "gzip_comp_level"; then echo "gzip_comp_level 5;"; fi
        if ! nginx_has "gzip_min_length"; then echo "gzip_min_length 1024;"; fi
        if ! nginx_has "gzip_types"; then
...
# NOTE: gzip must be enabled in /etc/nginx/nginx.conf (http{} block).
# This optimizer include intentionally does NOT set `gzip on;` to avoid duplicates.

...
#
# Usage:
#   sudo -E bash drupal_optimizerV0.2.5.8.sh
#   sudo -E PROD_STRICT=1 bash drupal_optimizerV0.2.5.8.sh     # production-like OPcache (no timestamp checks)
#   sudo -E ENABLE_DRUPAL_PERF_CONFIG=0 bash drupal_optimizerV0.2.5.8.sh  # skip drush config:set tuning
#
...
ENABLE_ACL_MODE="${ENABLE_ACL_MODE:-1}"  # default 1 to avoid files perms issues (use --no-acl to disable)
ENABLE_HARDEN_MODE="${ENABLE_HARDEN_MODE:-0}"
ENABLE_TRUSTED_HOSTS_FIX="${ENABLE_TRUSTED_HOSTS_FIX:-1}"  # auto-fix trusted_host_patterns once (recommended)
...
# Lock file path for cron flock. IMPORTANT: do NOT place lock directly under /run,
# because non-root services cannot create files there.
# We use RuntimeDirectory=drupal-cron and keep the lock under that directory.
CRON_LOCK_PATH="${CRON_LOCK_PATH:-/run/drupal-cron/drupal-cron.lock}"
...
Usage:
  sudo -E bash $(basename "$0") [--uri URL] [--acl|--no-acl] [--harden] [--fix-trusted-hosts|--no-fix-trusted-hosts] [--trusted-hosts LIST]
...
Options:
  ...
  --no-acl              Disable ACL mode (default: enabled).
  --harden              Protect sites/default (fixes "Not protected") and lock down settings files.
  --fix-trusted-hosts   Ensure trusted_host_patterns is configured (writes once to settings.statekeeper.php; recommended).
  --no-fix-trusted-hosts  Disable trusted host auto-fix for this run.
...
Environment toggles (examples):
  ...
  ENABLE_TRUSTED_HOSTS_FIX=1 Default (auto-fix trusted hosts once). Set 0 to disable.
...
if [[ $# -gt 0 ]]; then
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --fix-trusted-hosts) ENABLE_TRUSTED_HOSTS_FIX=1; shift;;
      --no-fix-trusted-hosts) ENABLE_TRUSTED_HOSTS_FIX=0; shift;;
...
install_cron_timer(){
  ...
  if [[ "${CRON_USE_FLOCK}" == "1" ]]; then
    flock_prefix="/usr/bin/flock -n ${CRON_LOCK_PATH}"
  fi

  # Build ExecStart line without leading blanks (systemd is picky).
  local execstart
  if [[ -n "${flock_prefix}" ]]; then
    execstart="${flock_prefix} ${WEB_ROOT}/vendor/bin/drush -y ${uri_arg} cron"
  else
    execstart="${WEB_ROOT}/vendor/bin/drush -y ${uri_arg} cron"
  fi
...
ensure_collab_group(){
  ...
  # Add users to group (best-effort, but only if not already a member to reduce churn).
  local added_users=()
  local u
  for u in "${APP_USER}" "${WEB_USER}"; do
    if id -u "${u}" >/dev/null 2>&1; then
      if ! id -nG "${u}" 2>/dev/null | tr ' ' '\n' | grep -Fxq "${COLLAB_GROUP}"; then
        usermod -aG "${COLLAB_GROUP}" "${u}" >/dev/null 2>&1 || true
        added_users+=("${u}")
      fi
    else
      warn "User not found: ${u} (skip group membership)"
    fi
  done
  if (( ${#added_users[@]} > 0 )); then
    warn "Added users to group '${COLLAB_GROUP}': ${added_users[*]}. Existing SSH sessions won't see it until you re-login (or run: newgrp ${COLLAB_GROUP}). If you hit drush 'Permission denied' on files/, enable --acl."
  fi
...
# Verify APP_USER can create+delete assets in files/css (needed for drush cr / theme enable).
verify_files_css_unlink_as_app() {
  local files_dir="$1"
  local css_dir="${files_dir}/css"
  mkdir -p "${css_dir}" || true

  local tmp="${css_dir}/.__perm_test_${RANDOM}_$$_$(date +%s).css"

  # Try as APP_USER (not as root). Unlink needs write on directory.
  if run_as_app_drupal bash -lc "set -e; umask 002; echo '/* perm test */' > '$tmp'; rm -f '$tmp'" >>"$LOG_FILE" 2>&1; then
    info "ACL/perms OK: ${APP_USER} can unlink in ${css_dir}"
    return 0
  fi

  warn "ACL/perms issue: ${APP_USER} cannot unlink in ${css_dir}. Re-applying perms+ACL."
  fix_runtime_files_perms
  apply_acl_on_files_dir "${files_dir}"

  # Try again; if still fails, print diagnostics.
  if ! run_as_app_drupal bash -lc "set -e; umask 002; echo '/* perm test */' > '$tmp'; rm -f '$tmp'" >>"$LOG_FILE" 2>&1; then
    warn "Still cannot unlink in ${css_dir}. Diagnostics:"
    stat -c '%A %a %U:%G %n' "${files_dir}" "${css_dir}" >>"$LOG_FILE" 2>&1 || true
    getfacl -p "${css_dir}" >>"$LOG_FILE" 2>&1 || true
    return 1
  fi

  info "ACL/perms fixed: ${APP_USER} can unlink in ${css_dir}"
  return 0
}
...
fix_runtime_files_perms(){
  ...
  if [[ -d "${files_dir}" ]]; then
    # Pre-create asset aggregate dirs so they inherit the right group/setgid/ACL before the first browser request.
    mkdir -p "${files_dir}/css" "${files_dir}/js" 2>/dev/null || true
...
redis_stage2_drupal(){
  ...
  # Enable module.
  if ! drush_run en redis -y >>"$LOG_FILE" 2>&1; then
    warn "Redis module enable failed; skipping Redis integration to avoid breaking the site."
    echo "pending" >"$pending" 2>/dev/null || true
    return 0
  fi

  # Write settings.redis.php (atomic + lint). Never allow shell expansion of $settings.
  local redis_settings="${WEB_ROOT}/web/sites/default/settings.redis.php"
  local tmp_redis
  tmp_redis="$(mktemp "${redis_settings}.tmp.XXXX")"

  cat >"$tmp_redis" <<'PHP'
<?php
/**
 * Redis settings (generated by drupal_optimizer __SCRIPT_VERSION__).
 *
 * Safety notes:
 * - This file is linted (php -l) before being moved into place.
 * - It is included only when it exists (see settings.statekeeper.php).
 * - Disable Redis quickly by renaming/removing this file.
 */

$settings['redis.connection']['interface'] = 'PhpRedis';
$settings['redis.connection']['host'] = getenv('DRUPAL_REDIS_HOST') ?: '127.0.0.1';
$settings['redis.connection']['port'] = (int) (getenv('DRUPAL_REDIS_PORT') ?: 6379);

// Use Redis for the default cache backend.
$settings['cache']['default'] = 'cache.backend.redis';

// Optional: cache prefix (useful if sharing Redis across sites).
$settings['cache_prefix']['default'] = getenv('DRUPAL_REDIS_PREFIX') ?: 'drupal';

// Container services provided by the redis module.
$settings['container_yamls'][] = 'modules/contrib/redis/example.services.yml';
PHP

  # Inject version string (keeping heredoc quoted so $settings is not expanded).
  sed -i "s/__SCRIPT_VERSION__/${SCRIPT_VERSION}/g" "$tmp_redis" >>"$LOG_FILE" 2>&1 || true

  if ! php -l "$tmp_redis" >/dev/null 2>&1; then
    rm -f "$tmp_redis" 2>/dev/null || true
    fail "Generated settings.redis.php failed PHP lint (site left unchanged)."
  fi

  chown "${APP_USER}:${COLLAB_GROUP}" "$tmp_redis"
  chmod 0640 "$tmp_redis"
  mv -f "$tmp_redis" "$redis_settings"
  ...
  # Defensive: remove ALL existing blocks (regardless of surrounding newlines), then append exactly one canonical block.
  run_as_root perl -0777 -i -pe "s/\s*\Q${begin}\E.*?\Q${end}\E\s*/\n/sg" "$sk" >>"$LOG_FILE" 2>&1 || true
...
EOF
  ...
  dedupe_managed_blocks_file "$sk" || true


  chown "${APP_USER}:${COLLAB_GROUP}" "$sk" 2>/dev/null || true
  chmod 0640 "$sk" 2>/dev/null || true

  # Defensive: validate statekeeper file after edits so Drush/Drupal won't brick on bootstrap.
  ensure_statekeeper_file
...
ensure_statekeeper_file(){
  ...
  # - remove UTF-8 BOM
  # - ensure EXACTLY ONE '<?php' opening tag at the top (avoid duplicate tags that cause ParseError)
  ...
  if command -v php >/dev/null 2>&1; then
    if ! php -l "$f" >/dev/null 2>&1; then
      ...
      printf "<?php\n/**\n * Auto-repaired placeholder (invalid prior file).\n */\n" > "$f"
    fi
  fi
...
settings_has_statekeeper_include(){
  ...
  grep -q 'settings.statekeeper.php' "${settings}" 2>/dev/null
}

ensure_statekeeper_include(){
  local settings="$WEB_PUBLIC/sites/default/settings.php"

  # Safety: clean any duplicated managed blocks first (keeps last copy)
  dedupe_managed_blocks_file "$settings" || true

  if grep -qF "# BEGIN STATEKEEPER INCLUDE (managed by drupal_optimizer)" "$settings" 2>/dev/null; then
    ok "settings.php already includes statekeeper include block."
    return 0
  fi

  step "Appending statekeeper include block to settings.php (as ${APP_USER})"

  local tmp settings_q tmp_q
  tmp="$(mktemp /tmp/statekeeper-include.XXXXXX)"
  cat >"$tmp" <<'PHP'

# BEGIN STATEKEEPER INCLUDE (managed by drupal_optimizer)
...
PHP
  chown "${APP_USER}:${COLLAB_GROUP}" "$tmp" || true
  chmod 0644 "$tmp" || true

  settings_q="$(printf %q "$settings")"
  tmp_q="$(printf %q "$tmp")"
  run_as_app_drupal bash -lc "cat $tmp_q >> $settings_q"
  rm -f "$tmp" || true

  dedupe_managed_blocks_file "$settings" || true
}

has_active_trusted_host_assignment(){
  local f="$1"
  grep -Eq '^[[:space:]]*\$settings\[[[:space:]]*['\''"]trusted_host_patterns['\''"][[:space:]]*\]' "$f" 2>/dev/null
}
...
write_trusted_hosts(){
  ...
  # One-time behavior: if already configured, do nothing.
  if has_active_trusted_host_assignment "${settings}"; then
    ok "trusted_host_patterns already configured in settings.php; skipping."
    return 0
  fi
  if has_active_trusted_host_assignment "${f}"; then
    ok "trusted_host_patterns already configured in settings.statekeeper.php; skipping."
    return 0
  fi
  ...
  # Remove previous block (optimizer/statekeeper variants).
  run_as_root perl -0777 -i -pe 's#\n(?:(?://|\#)\s*BEGIN\s+(?:OPTIMIZER|STATEKEEPER)\s+TRUSTED\s+HOSTS.*?)(?:(?://|\#)\s*END\s+(?:OPTIMIZER|STATEKEEPER)\s+TRUSTED\s+HOSTS)\s*\n##sg' "${f}" || true

  # Append new block.
  cat >> "${f}" <<EOF

# BEGIN STATEKEEPER TRUSTED HOSTS (managed by drupal_optimizer)
\$settings['trusted_host_patterns'] = [
${php_array}];
# END STATEKEEPER TRUSTED HOSTS
EOF

  dedupe_managed_blocks_file "$f" || true
...
write_file_chmod_settings(){
  local f="${WEB_PUBLIC}/sites/default/settings.statekeeper.php"
  [[ -f "$f" ]] || return 0

  step "Writing file_chmod_* into settings.statekeeper.php (keep setgid on new dirs)"
  # Remove previous block if present.
  sed -i '/^# BEGIN DRUPAL_TOOLS_FILE_CHMOD/,/^# END DRUPAL_TOOLS_FILE_CHMOD/d' "$f" 2>/dev/null || true

  cat >>"$f" <<'EOF'

# BEGIN DRUPAL_TOOLS_FILE_CHMOD
$settings['file_chmod_directory'] = 02775;
$settings['file_chmod_file'] = 0664;
# END DRUPAL_TOOLS_FILE_CHMOD
EOF
...
apply_trusted_hosts_fix(){
  [[ "${ENABLE_TRUSTED_HOSTS_FIX}" == "1" ]] || { info "Trusted host auto-fix disabled."; return 0; }
  local settings="${WEB_PUBLIC}/sites/default/settings.php"
  [[ -f "${settings}" ]] || { info "Trusted hosts: settings.php missing (site not installed yet); skip."; return 0; }
  ensure_statekeeper_include
  ensure_statekeeper_file
  write_trusted_hosts
  write_file_chmod_settings
...
main(){
  ...
  verify_files_css_unlink_as_app "${WEB_PUBLIC}/sites/default/files" || true

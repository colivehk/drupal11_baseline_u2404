...
# - Optional ACL mode (disabled by default)
...
ENABLE_ACL="${ENABLE_ACL:-0}"
print_help(){
  cat <<'EOF'
Usage:
  sudo -E bash drupal_installV7.5.6.sh [--fix-perms] [--acl|--no-acl]

Options:
  --fix-perms   Only normalize ownership/permissions on existing paths (no apt, no install, no mkdir)
  --acl         Apply POSIX ACLs on sites/default/files so OPS_USER can delete css/js even if your shell group list wasn't refreshed.
...
########################################
# Optional ACL mode (prevents "Permission denied" when your shell group list wasn't refreshed)
########################################
ensure_acl_tools() {
  command -v setfacl >/dev/null 2>&1 && command -v getfacl >/dev/null 2>&1 && return 0
  if [[ "${FIX_PERMS_ONLY}" == "1" ]]; then
    warn "ACL requested but setfacl/getfacl not installed and --fix-perms is pure. Skipping ACL. (Install 'acl' package or run full install/optimizer.)"
    return 1
  fi
  step "Installing package: acl (for setfacl/getfacl)"
  apt_install acl || { warn "Failed installing acl; skipping ACL."; return 1; }
  return 0
}

apply_acl_on_files_dir() {
  [[ "${ENABLE_ACL}" == "1" ]] || return 0
  local files_dir="$1"
  [[ -d "${files_dir}" ]] || { warn "ACL: directory not found: ${files_dir}"; return 0; }
  ensure_acl_tools || return 0

  step "ACL mode: grant ${OPS_USER} + ${WEB_USER} rwX on ${files_dir} (and default ACL) to avoid group-refresh issues"
  # Access ACL recursively
  setfacl -R -m "u:${OPS_USER}:rwX,u:${WEB_USER}:rwX,g:${COLLAB_GROUP}:rwX,m::rwX" "${files_dir}" >>"$LOG_FILE" 2>&1 || true
  # Default ACL must be set on directories (so new files inherit)
...
normalize_permissions() {
  ...
  if [[ -d "${WEB_ROOT}" ]]; then
    ...
    apply_acl_on_files_dir "${WEB_ROOT}/sites/default/files"
  else

...
SCRIPT_VERSION="${SCRIPT_VERSION:-V7.5.10.3}"
...
print_help(){
  cat <<'EOF'
Usage:
  sudo -E bash drupal_installV7.5.10.3.sh [--fix-perms] [--acl]
...
Notes:
  ...
  - Full install mode installs 'acl' package so setfacl/getfacl are available (ACL is enabled by default)
...
ensure_file_chmod_fallback(){
  local settings="${WEB_ROOT}/sites/default/settings.php"
  [[ -f "${settings}" ]] || return 0

  step "Ensuring file_chmod_* defaults in settings.php (keep setgid on new dirs)"
  local tmp
  tmp="$(mktemp /tmp/settings.php.XXXXXX)"

  # Remove our previous block if present (idempotent).
  perl -0777 -pe 's#\n\# BEGIN DRUPAL_INSTALL_FILE_CHMOD.*?\# END DRUPAL_INSTALL_FILE_CHMOD\s*\n##sg' \
    "${settings}" > "${tmp}" || cp -a "${settings}" "${tmp}"

  cat >>"${tmp}" <<'EOF'

# BEGIN DRUPAL_INSTALL_FILE_CHMOD
$settings['file_chmod_directory'] = 02775;
$settings['file_chmod_file'] = 0664;
# END DRUPAL_INSTALL_FILE_CHMOD
EOF

  # Replace with correct owner/perm (avoid sed -i ownership surprises).
  install -o "${CODE_OWNER}" -g "${COLLAB_GROUP}" -m 0640 "${tmp}" "${settings}" 2>/dev/null || true
...
if [[ "${FIX_PERMS_ONLY}" == "1" ]]; then
  ...
  normalize_permissions_pure
# Also ensure runtime ACL + file_chmod defaults are in place (no directory creation).
if [[ "${ENABLE_ACL}" == "1" ]] && command -v setfacl >/dev/null 2>&1; then
  apply_acl_runtime_files
fi
ensure_file_chmod_fallback
...
# Ensure file_chmod defaults are present even on first page load.
ensure_file_chmod_fallback

...
print_help(){
  cat <<'EOF'
Usage:
  sudo -E bash drupal_installV7.5.2.sh [--fix-perms]
...
Notes:
  - Root is required.
  - Default model: CODE_OWNER=ubuntu, APP_USER=www-data, COLLAB_GROUP=drupal
  - Override via env vars: CODE_OWNER=..., APP_USER=..., COLLAB_GROUP=...
EOF
}
...
# Ensure collaboration group exists and both ${CODE_OWNER} and ${APP_USER} are members.
ensure_collab_group() {
  ...
  if id -u "${APP_USER}" >/dev/null 2>&1; then
    usermod -aG "${COLLAB_GROUP}" "${APP_USER}" || true
  else
    warn "APP_USER '${APP_USER}' does not exist; skip group membership."
  fi

  if id -u "${CODE_OWNER}" >/dev/null 2>&1; then
    usermod -aG "${COLLAB_GROUP}" "${CODE_OWNER}" || true
  else
    warn "CODE_OWNER '${CODE_OWNER}' does not exist; skip group membership."
  fi
}

# Ensure fixed Drupal project root exists and matches the V7.5.2 ownership model.
ensure_drupal_root() {
  ensure_collab_group
  ...
  # Composer runtime dirs for ${APP_USER} (who runs composer/drush)
  install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "/var/www/.config"
  install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "${COMPOSER_HOME_DIR}"
  install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "/var/www/.cache"
  install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "/var/www/.cache/composer"
}
...
# - Runtime files/: APP_USER:COLLAB_GROUP with group-writable bits
normalize_permissions() {
  ...
  if [[ -d "${WEB_ROOT}" ]]; then
    install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "${WEB_ROOT}/sites/default/files"
    chown -R "${APP_USER}:${COLLAB_GROUP}" "${WEB_ROOT}/sites/default/files" || true
  ...
  # Composer cache/home
  chown -R "${APP_USER}:${COLLAB_GROUP}" "${COMPOSER_HOME_DIR}" "/var/www/.cache/composer" 2>/dev/null || true
  find "${COMPOSER_HOME_DIR}" "/var/www/.cache/composer" -type d -exec chmod 2775 {} \; 2>/dev/null || true
  find "${COMPOSER_HOME_DIR}" "/var/www/.cache/composer" -type f -exec chmod g+rw {} \; 2>/dev/null || true
...
normalize_permissions_pure() {
  ...
  if [[ -d "${files_dir}" ]]; then
    chown -R "${APP_USER}:${COLLAB_GROUP}" "${files_dir}" || true
  ...
  for d in "${COMPOSER_HOME_DIR}" "/var/www/.cache/composer"; do
    if [[ -d "${d}" ]]; then
      chown -R "${APP_USER}:${COLLAB_GROUP}" "${d}" 2>/dev/null || true
...
as_app_user_drupal() {
  ...
  su -s /bin/bash - "${APP_USER}" -c '
    ...
    cd "$3" || exit 1
    bash -c "$4"
  ' bash "${COMPOSER_HOME_DIR}" "/var/www/.cache/composer" "${DRUPAL_DIR}" "${cmd}"
}
...
_log "$C_BOLD" "=== Drupal ${DRUPAL_VERSION} install ${SCRIPT_VERSION} (V7.5.2 collab perms) ==="
...
if [[ "${FIX_PERMS_ONLY}" == "1" ]]; then
  ...
  info "Tip: after fixing perms, run cache rebuild as ${APP_USER}:"
  info "  sudo -u ${APP_USER} -E ${DRUPAL_DIR}/vendor/bin/drush --uri=${SITE_URI} cr"
  ok "Done (--fix-perms)."
  exit 0
fi
...
normalize_permissions
ok "Permissions baseline applied (CODE_OWNER=${CODE_OWNER}, APP_USER=${APP_USER}, GROUP=${COLLAB_GROUP})."
...
# Ensure collaboration/dirs exist (do NOT change /var/www ownership to ${APP_USER}).
ensure_drupal_root
normalize_permissions

# Ensure XDG-style composer home exists (Composer 2 uses ~/.config/composer by default)
install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "$(dirname "${COMPOSER_HOME_DIR}")"
install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "${COMPOSER_HOME_DIR}"
install -d -m 02775 -o "${APP_USER}" -g "${COLLAB_GROUP}" "/var/www/.cache/composer"
...
if COMPOSER_DIAG="$(
  as_app_user_drupal "env -i HOME=\"/var/www\" PATH=\"${SAFE_PATH}\" COMPOSER_HOME=\"${COMPOSER_HOME_DIR}\" COMPOSER_CACHE_DIR=\"/var/www/.cache/composer\" ${COMPOSER_BIN} diagnose --no-ansi"
)"; then
  DIAG_EC=0
else
  DIAG_EC=$?
...
if [[ $DIAG_EC -ne 0 ]]; then
  ...
  warn "Common causes:"
  warn "  - git not found in PATH for ${APP_USER} (check: sudo -u ${APP_USER} -H bash -lc 'command -v git')"
  ...
  PUBBLOCK="$(printf "%s\n" "${COMPOSER_DIAG}" | awk '
    /^Checking pubkeys:/{inblk=1; print; next}
    inblk && /^Checking /{exit}
    inblk{print}
  ' | head -n 12)"

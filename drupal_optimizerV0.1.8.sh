...
#
# Usage:
#   sudo -E bash drupal_optimizerV0.1.8.sh
#   sudo -E PROD_STRICT=1 bash drupal_optimizerV0.1.8.sh     # production-like OPcache (no timestamp checks)
#   sudo -E ENABLE_DRUPAL_PERF_CONFIG=0 bash drupal_optimizerV0.1.8.sh  # skip drush config:set tuning
#
...
Usage:
  sudo -E bash $(basename "$0") [--acl|--no-acl]
...
# Args (lightweight): only for ACL helper
if [[ $# -gt 0 ]]; then
  while [[ $# -gt 0 ]]; do
    case "$1" in
...
load_creds(){
  ...
  info "OPS_USER(APP_USER)=${APP_USER}"
...
fix_runtime_files_perms(){
  ...
  local files_dir="${WEB_ROOT}/web/sites/default/files"
  if [[ -d "${files_dir}" ]]; then
...
EOF" >>"$LOG_FILE" 2>&1
  ...
  # Ensure settings.php includes settings.redis.php (append with markers if missing).
  if ! settings_has_redis_include; then
    step "Adding Redis include block to settings.php (as ${APP_USER})"
    run_as_app_drupal bash -lc "cat >> $(printf %q "$settings") <<'EOF'

# BEGIN DRUPAL_OPTIMIZER_REDIS
if (file_exists(\$app_root . '/' . \$site_path . '/settings.redis.php')) {
  include \$app_root . '/' . \$site_path . '/settings.redis.php';
}
# END DRUPAL_OPTIMIZER_REDIS
EOF" >>"$LOG_FILE" 2>&1
  else
    info "settings.php already has Redis include markers (skip)"
  fi

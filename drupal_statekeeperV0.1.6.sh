...
ensure_settings_statekeeper_file() {
  ...
  if [[ ! -f "$f" ]]; then
    ...
    {
      echo "<?php"
      echo "// settings.statekeeper.php (managed by drupal_statekeeper)"
    } > "$f"
  else
    # Ensure it begins with <?php
    if ! head -n 1 "$f" | grep -q "^<\?php"; then
      warn "${f} missing PHP opening tag; prepending '<?php'."
      local tmp
      tmp="$(mktemp)"
      {
        echo "<?php"
